...
router.post('/submit', requireAuth, requireRole('candidate'), async (req: Request, res: Response) => {
  try {
...
router.get('/my-applications', requireAuth, requireRole('candidate'), async (req: Request, res: Response) => {
  try {
    // Get candidate ID from user session
    const candidateRepo = AppDataSource.getRepository(Candidate);
    const candidate = await candidateRepo.findOne({ 
...
router.get('/for-review', requireAuth, requireRole('lecturer'), async (req: Request, res: Response) => {
  try {
    ...
    const courseIds = user.lecturer.courses?.map(course => course.id) || [];
    ...
    // Use IN clause instead of mapping
    ...
    const formattedApplications = applications.map(app => ({
      ...
      previousRoles: app.candidate.previous_roles?.map(role => ({
        ...
        position: role.position,
        organisation: role.organisation,
        startDate: role.start_date ? (role.start_date instanceof Date ? role.start_date.toISOString().split('T')[0] : String(role.start_date)) : undefined,
        endDate: role.end_date ? (role.end_date instanceof Date ? role.end_date.toISOString().split('T')[0] : String(role.end_date)) : undefined,
        description: role.description
      })) || []
    }));
...
router.put('/:id/status', requireAuth, requireRole('lecturer'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.post('/:id/comment', requireAuth, requireRole('lecturer'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.put('/:id/ranking', requireAuth, requireRole('lecturer'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
